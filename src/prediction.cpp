...
map<int, vector<Vehicle>> Prediction::predict(const vector<vector<double>> &sensor_fusion)
{
    ...
    for (int i = 0; i < sensor_fusion.size(); i++)
    {
        ...
        predictions[sf.id] = predict_trajectory(v);
    }
    return predictions;
}
 ...
 */
vector<Vehicle> Prediction::predict_trajectory(const Vehicle& v)
{
    ...
    {
        car_s += ds;
        auto xy = road.get_xy(car_s, v.d);
        auto pred = Vehicle(xy[0], xy[1], car_s, v.d, v.yaw, v.speed);
